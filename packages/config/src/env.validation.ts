...
const isPort = (v: string): boolean => /^\d+$/.test(v) && parseInt(v, 10) > 0 && parseInt(v, 10) <= 65535;

const hasMinLength = (min: number) => (v: string): boolean => v.length >= min;
...
export function validateEnv(): ValidationResult {
  ...
  for (const rule of rules) {
    ...
    if (rule.required && !value) {
      result.errors.push(
        `[MISSING] ${rule.label || rule.key}: ${rule.key} is required but not set`,
      );
      result.passed = false;
      continue;
    }
    ...
    if (value && rule.validate && !rule.validate(value)) {
      if (rule.key === 'JWT_SECRET' || rule.key === 'JWT_REFRESH_SECRET') {
        result.errors.push(
          `[INVALID] ${rule.label || rule.key}: ${rule.key} must be at least 32 characters (currently ${value.length}). ` +
          `Generate a secure secret: openssl rand -base64 64`,
        );
        result.passed = false;
      } else {
        result.warnings.push(
          `[WARN] ${rule.label || rule.key}: ${rule.key} appears malformed: "${value.substring(0, 40)}${value.length > 40 ? '...' : ''}"`,
        );
      }
    }
    ...
    if (
      ...
      const isCritical = ['JWT_SECRET', 'JWT_REFRESH_SECRET', 'DATABASE_URL'].includes(
        rule.key,
      );
      if (isCritical) {
        result.errors.push(
          `[PLACEHOLDER] ${rule.label || rule.key}: ${rule.key} still has a placeholder value. Set a real value before deploying.`,
        );
        result.passed = false;
      } else {
        result.warnings.push(
          `[PLACEHOLDER] ${rule.label || rule.key}: ${rule.key} has a placeholder value.`,
        );
      }
    }
  }
  ...
  if (!process.env.SMTP_HOST && !process.env.SMTP_PORT) {
    result.warnings.push(
      '[WARN] SMTP not configured — email sending will silently fail. Set SMTP_HOST and SMTP_PORT.',
    );
  }
  ...
  if (process.env.JWT_SECRET && process.env.JWT_SECRET.length < 64) {
    result.warnings.push(
      `[WARN] JWT_SECRET is only ${process.env.JWT_SECRET.length} chars. Use 64+ chars for production: openssl rand -base64 64`,
    );
  }
